...
namespace Report
{
    ...
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        private string _TB_Log_Text { get; set; }

        public string TB_Log_Text
        {
            get
            {
                return _TB_Log_Text;
                            ...
                            for (int nCnt = 0; nCnt < Person_items.Count; nCnt++)
                            {
                                if (Person_items[nCnt].bSelected == true)
                                {
                                    ...
                                    Application.Current.Dispatcher.Invoke(() =>
                                    {
                                        ...
                                        mvmodel.RemoveData(Person_items[nCnt].sName,
                                        Person_items[nCnt].sAge,
                                        Person_items[nCnt].sPhone);
                                        _Person_items.Remove(Person_items[nCnt]);
                                    });
                                }
                            }
                        });
        ...
        private void SearckBtnClickCommandAction(object obj)
        {
                ...
                string.IsNullOrWhiteSpace(_SelectedLB_item))    // ALL
            {
                ...
                thread = new Thread(() =>
                {
                    var temp = mvmodel.GetSelectData();    
                    Application.Current.Dispatcher.Invoke(() =>
                    {
                        Task.Delay(nDelayTime);
                        _Person_items.Clear();
            ...
            else
            {
                Thread t = new Thread(() =>
                    {
                        var mimi =mvmodel.GetSelectDatas(_SelectedLB_item, _sKeyword);
                        Application.Current.Dispatcher.Invoke(() =>
                        {
                            Task.Delay(nDelayTime);
                            Person_items.Clear();
        ...
        public void Log(string msg)
        {
            try
            {
                ...
                sw.Close();
                Printf(sConent);
                _TB_Log_Text = _TB_Log_Text +Environment.NewLine+sConent;
            }
            catch(Exception ex)
            {
                Log(string.Format("{0}\n\r{1}", ex.Message, ex.StackTrace));
